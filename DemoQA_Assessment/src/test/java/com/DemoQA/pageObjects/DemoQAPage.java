package com.DemoQA.pageObjects;

import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.interactions.Actions;
import org.openqa.selenium.support.FindBy;
import org.openqa.selenium.support.PageFactory;

public class DemoQAPage {
	

    private WebDriver driver;

    // Constructor initializes WebDriver instance and elements
    public DemoQAPage(WebDriver driver) {
        this.driver = driver;
        PageFactory.initElements(driver, this);
    }
    
    
    public By FirstAlert = By.id("alertButton");
    public By SecondAlert = By.id("timerAlertButton");
    public By ThirdAlert = By.id("confirmButton");
    public By ForthAlert = By.id("promtButton");

    public By confirmResult = By.id("confirmResult");
    public By promptResult = By.id("promptResult");
    
    
    public By Accept = By.xpath("//*[@id='droppableExample-tab-accept']");
    
    public By source = By.xpath("//*[@id='notAcceptable']");
	 
	public By target = By.xpath("(//*[@id='droppable'])[2]");
	
	public By NewWindowBTN = By.xpath("//*[@id = 'windowButton']");

	public By NewWindowText = By.xpath("//*[@id = 'sampleHeading']");


    
   
	public void click_FirstAlert()
	{
		WebElement First_Alert = driver.findElement(FirstAlert);	
		First_Alert.click();
	}
	
	public void click_SecondAlert()
	{
		WebElement Second_Alert = driver.findElement(SecondAlert);	
		Second_Alert.click();
	}
	public void click_ThirdAlert()
	{
		WebElement Third_Alert = driver.findElement(ThirdAlert);	
		Third_Alert.click();
	}
	public void click_ForthAlert()
	{
		WebElement Forth_Alert = driver.findElement(ForthAlert);	
		Forth_Alert.click();
	}
	
	public String Get_Confirmation_Text()
	{
		WebElement confirm_Result = driver.findElement(confirmResult);	
		return confirm_Result.getText();
	}
	
	
	public String Get_promptResult_Text()
	{
		WebElement prompt_Result = driver.findElement(promptResult);	
		return prompt_Result.getText();
	}
	
	
	public WebElement get_source()
	{
		 WebElement sourceEle = driver.findElement(source);
		return sourceEle;
	}
	
	public WebElement get_target()
	{
		  WebElement targetEle = driver.findElement(target);
		return targetEle;
	}
	
	
	public void click_Accept_tab()
	{
		 WebElement AcceptEle = driver.findElement(Accept);
		 AcceptEle.click();
	}
	
	

	public void click_NewWindowBTN()
	{
		 WebElement NewWindow_BTN = driver.findElement(NewWindowBTN);
		 NewWindow_BTN.click();
	}

	
	public String Get_NewWindow_Text()
	{
		WebElement NewWindow_Text = driver.findElement(NewWindowText);	
		return NewWindow_Text.getText();
	}
		
	
}
